...
Future<void> cetakAduanTik(Map<String, dynamic> aduan) async {
  final pdf = pw.Document();

  // Fungsi bantu agar aman dari null
  ...
  // ===== Tanggal proses =====
  final tanggalAnalisis = safe(
    aduan["tanggal_analisa"] ?? aduan["created_at"] ?? aduan["analyze_date"],
  );
  final tanggalTindakLanjut = safe(
    aduan["tanggal_tindak"] ?? aduan["updated_at"] ?? aduan["followup_date"],
  );
  final tanggalHasil = safe(
    aduan["tanggal_selesai"] ?? aduan["tanggal_hasil"] ?? aduan["result_date"],
  );

  // ===== Tambahan =====
  final nup = safe(aduan["nup"]);
  final kondisi = safe(aduan["kondisi"]);
  final kategori = safe(aduan["kategori"] ?? aduan["jenis_kerusakan"]);

  // ===== Nomor Formulir (pojok kiri atas) =====
  const nomorFormulir =
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        ...
        pw.Center(
          child: pw.Column(
            children: [
              ...
              pw.Text(
                "Surat Permintaan Perbaikan Barang TIK",
                style: pw.TextStyle(
                  fontSize: 14,
                  fontWeight: pw.FontWeight.bold,
        ...
        if (nup != "-") _infoRow("NUP", nup),
        if (kondisi != "-") _infoRow("Kondisi", kondisi),
        if (kategori != "-") _infoRow("Kategori Kerusakan", kategori),
